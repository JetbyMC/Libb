...
public class ParsedGui extends PaginatedGui {
    ...
    public void buildItems(List<Item> items) {
        ...
        for (Map.Entry<Integer, List<Item>> entry : slotCandidates.entrySet()) {
            ...
            for (Item candidate : sorted) {
                if (RequirementEvaluator.meetsAll(viewer, applyPlaceholders(candidate.viewRequirements()))) {
    ...
    public ParsedGui setReplace(Item item, String key, String input) {
        Map<String, String> placeholders = individualPlaceholders.get(item);
        if (placeholders == null) {
            individualPlaceholders.put(item, Map.of(key, input));
        } else {
            placeholders.put(key, input);
        }
        return this;
